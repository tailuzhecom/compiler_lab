...
/* 语法规则 */
//函数定义
func: type_dec IDENTITY LPAREN args_list RPAREN LBRACKET statement RBRACKET { printf("func: %s\n", $2); }
    ;
...
args_list: type_dec IDENTITY { printf("arg name: %s\n", $2); }
	 ...
	 ;
// 类声明
class_decl: CLASS IDENTITY LBRACKET class_decl_stmt_list RBRACKET ';' { printf("class %s\n", $2); }
...
// 变量声明，允许初始化时赋值
decl_stmt: type_dec id_list { printf("declare statement\n"); }
    | type_dec assign_stmt  { printf("declare statement & assignment\n"); }
    ;


// 全局变量声明
global_decl_stmt: decl_stmt ';' { printf("global declare statement\n"); }
		;
...
// if语句
if_stmt: IF LPAREN expr RPAREN LBRACKET statement RBRACKET { printf("if statement\n"); }
       | IF LPAREN expr RPAREN LBRACKET statement RBRACKET ELSE LBRACKET statement RBRACKET { printf("if-else statement\n"); }
       ;

// while语句
while_stmt: WHILE LPAREN expr RPAREN LBRACKET statement RBRACKET { printf("while statement\n"); }
          ;

// switch语句
switch_stmt: SWITCH LPAREN expr RPAREN LBRACKET case_list RBRACKET { printf("switch statement\n"); }
	   ;
// for语句
for_stmt: FOR LPAREN decl_stmt ';' expr ';' assign_stmt RPAREN LBRACKET statement RBRACKET { printf("for statement\n"); }

// call语句
call_stmt: IDENTITY LPAREN expr_list RPAREN { printf("call %s\n", $1); }
...
// return语句
return_stmt: RETURN expr { printf("return statement\n"); } 

// break语句
break_stmt: BREAK { printf("break statement\n"); }
